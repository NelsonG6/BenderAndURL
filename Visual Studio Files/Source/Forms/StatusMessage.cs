    ...
    class StatusMessage
    {
        ...
        public StatusMessage(AlgorithmState set_from)
        {
            ...
            if (!program_launch_message)
            {
                ...
                complete_message += (set_from.board_data.GetUnitSquare[Unit.Bender].x + 1).ToString();
                complete_message += ", " + (set_from.board_data.GetUnitSquare[Unit.Bender].y + 1).ToString() + ").";
            }
            ...
            else if (set_from.GetStepNumber() == 0)
            {
                ...
                complete_message += " at position (" + (set_from.board_data.GetUnitSquare[Unit.Bender].x + 1).ToString();
                complete_message += ", " + (set_from.board_data.GetUnitSquare[Unit.Bender].y + 1).ToString() + ").";
            ...
            else
            {
                ...
                new_position_data = "The resulting position was (" + (set_from.location_result[0] + 1).ToString();
                new_position_data += ", " + ((set_from.location_result[1]) + 1).ToString() + ").";

                //New percept
                
                new_percept_data = "The percept at the new location is: " + System.Environment.NewLine + set_from.bender_perception_ending.ToString() + ".";
